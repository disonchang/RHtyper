...
def cbs_stat(x):
    ...
    max_t=(s1-s0)**2*n/(i1-i0+1)/(n+1-i1+i0)
    max_start=i0
    max_end=i1+1    
...
def rsegment(x, start, end, L=[], shuffles=1000, p=.05):
    ...
    print('[rsegment]', 'Proposed partition of {} to {} from {} to {} with t value {} is {}'.format(start, end, start+s, start+e, t, threshold))
...
def segment(x, shuffles=10, p=.05):
...
if __name__ == '__main__':
    ...
    sample = generate_normal_time_series(2, minl=5, maxl=6)
    #print sample
    #cbs_stat(sample)
    #max_tstat(sample)
    #status, max_t, mas_s, max_e=cbs(sample)
    #print status, max_t, mas_s, max_e
    #status, max_t, mas_s, max_e, perm_p=my_cbs(sample)
    #print status, max_t, mas_s, max_e, perm_p
